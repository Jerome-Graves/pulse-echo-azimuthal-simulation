...
| beam_local_break.npz | 3 KB | ARCHIVE | sim/analysis/beam_local_break.py | Separability stress test of the beam-local predictor: 12-tessellation first-rank counts per window and estimator (env/tf), raw (ens_*) and after regressing the orientation-blind column geometry out of the predictor (ensr_*). Backs S10. |
...
| cross_fabric_coda_rows.npy | 42 KB | ARCHIVE | sim/analysis/cross_fabric.py | Pickled object array (needs allow_pickle=True) of 9 rows (sweep name, solver group, azimuths, measured coda power, predicted sigma_d^2 per azimuth) for the stress test that the coda level tracks 10log10(sigma_d^2) 1:1 across fabrics at fixed solver settings. Exploratory audit; not in the paper's claim map. |
