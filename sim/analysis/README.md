...
| S4 | single interface vs closed-form R (r = 0.966 over five contrasts; 6.7e-4 zero-contrast echo; spreading factor closes it to r = 0.995) | `sim/validation/two_grain_reflection.py`, `sim/validation/exact_rc.py` ... `exact_rc4.py` (exact anisotropic coefficient, Christoffel matching) | `sim/results/two_grain.npz`, `sim/results/exact_rc_psib_sweep.npz` (note: `exact_rc2/3.py` load `bicrystal_val.npz` from `Documents/pulse-echo-analysis-scratch`, outside the repo) |
| S4 | physical-optics absolute anchor (-88.1 dB direct vs -87.79 measured; band truncation 1.1 dB; +-3 dB band; dB-average disagreement 5.5 dB) | `physical_optics/po_src_1meas.py` ... `po_src_11final.py` (stepwise; `po_src_10cal.py` fixes the 3843 m/s delay mapping, `po_src_11final.py` the closed form, `po_src_7sens.py` the uncertainty band, `po_src_8robust.py` the glint concentration), `po_band_audit.py` (closes the estimator mismatch against the audited level) | `physical_optics/po_src_meas.npz`, `po_src_pred.npz`, `po_src_final.npz`, `po_src_dense.npz`, `po_src_diag.npz`, `po_src_shape.npz` |
| S5 | choice of reference and estimator; tab:reconcile (-81.09/-85.11/-87.79 dB); pedestal audit | `db_reconcile.py` (rebuilds the table on the audited local in-band estimator) | prints; levels re-derived from `out/sweeps` (`girdle_perp*`, `ladder_ppw10`, `lic_girdle_s11_ppw10`) |
| S5 | uniform-orientation control and contrast ladder (tab:ladder, intercept, n = 1.95) | `contrast_ladder.py`; rungs produced by `sim/runs/master_run.py` | sweeps `cs_f000/025/050/075_s11_ppw8`, `zerocontrast_ppw8`, `zc_s11_ppw6`, `zc_s11_ppw10`; `sim/results/bicrystal_ladder.npz` (master-run byproduct) |
| S5 | seed-23 ladder repeat (steps -3.72/-2.54 dB) | `eight_pairs_two_ladders.py`, `gate_refinement_audit.py` | sweeps `lad_girdle_s23_ppw6`, `mx_girdle_s23_ppw8`, `lad_girdle_s23_ppw10` |
...
| S7 | re-implementation, mirror and surrogate controls | `independent_check/sk_final.py` + `null_calibration.py` (shares no code with the predictor), `mirror_control.py`, `surrogates.py` | printed; earlier 44-candidate set |
| S8 | tab:ensemble, fabric main effect -2.86 dB, sign-flip null, split halves, specular seeds 7/17 | `eight_pairs_two_ladders.py` | `sim/results/ensemble.npz`; sweeps `mx_girdle_s*_ppw8`, `mx_single_s*_ppw8`, `girdle_perp_ppw8`, `singlemax_ppw8` |
...
| S9 | fabric-to-coda channel: speed route vs reflection-weighting route, wrong-fabric exchange, ablation table | `predict_field.py` + `_t2_common.py` (prediction variants full/geom/nodirec/flat), `facet_model/t2_final.py` (shared-tessellation wrong-fabric cases), `geometry_vs_fabric.py`, `facet_predictors.py` (born_spec/born_iso/geom_only decomposition). S9's text predates the restructure; its gain figures came from the scratch-era t2 studies | cached fields `t2p_*.npz` in `Documents/pulse-echo-analysis-scratch` (outside repo); `facet_model/t4build_s11_p8_k-8.npz` |
| S10 | axis null: bulk predictor rank 10/30, 22-observable panel FWER, beam-local predictor, matched-pair type contrast | `shift_null.py` (bulk predictor), `observable_panel.py` + `panel_fwer.py` (the panel; caches in `predcache/pred_*.npz`), `beam_local_windows.py`, `beam_local_admissibility.py`, `beam_local_verdict.py`, `beam_descriptors.py`, `eight_pairs_two_ladders.py` (third test) | `beam_local_windows.npz`, `beam_local_admissibility.npz`, `beam_local_break.npz`, `fabric_axis_windows.npz`, `faxwin_pred_iso_gcal.npz` |
...
| S12 | time-of-flight axis and concentration recovery, tab:axisrecovery, tab:azcount, girdle branch degeneracy | `tof_axis_recovery.py` (all three reported quantities), `channel_network/e1_axis.py` (adds seed 7, runs the same estimator), `channel_network/e1_selfcheck.py` (concentration-swing flag) | `tofaxis_build_s*_k*.npz` (here); fig:tofaz is the independent route, drawn by `sim/figures/fig_tofaz.py` from `fit_result.json` of sweeps `rigid_seed11`, `kappa8_seed17`, `oos_seed23` |
| S13 | frequency dependence 2 vs 5 MHz (-6.87 dB, residual ~ f^-1/2) | `frequency_scaling.py`; band-resolved follow-ups in `frequency/t3_atten.py`, `t3_level.py`, `t3_speckle.py` | `sim/results/freq_scaling.npz`; sweeps `fittest`, `prod5mhz` |
...
| Sec. 4.3 | position not crystallography: lag test (0.156 vs 0.062 at zero offset), 0.5 mm displacement kills first rank, identification switches on along the contrast ladder (f = 0 rank 7/48, uniform orientation 26/48) | `facet_model/t2_lag.py` (lag curve), `microstructure_error.py` (displacement), `zero_contrast_facet.py` + `analysis/validation/zc_sweep.py` (zero-contrast control), `centring_convention.py` (scores the ladder rungs), `identification_transfer.py` | `sim/results/microstructure_error.npz`, `sim/results/identification_transfer*.npz`; sweeps `cs_f*`, `zerocontrast_*`, `zc_s11_*` |
...
| Sec. 5 (discussion scope) | descriptor spreads, grain-size scope statement | `descriptor_spreads.py`, `channel_network/e3_grain.py`, `channel_network/e14_extras.py` | `channel_network/edge1.npz`, `edge1_a2k.npz`, `edge23.npz` |
