...
print(f"prod5mhz azimuths (n={len(AZ5)}): {AZ5[0]}..{AZ5[-1]} deg, "
...
res = {}
for nm, lab in (("fittest", "2 MHz"), ("prod5mhz", "5 MHz")):
    azs = AZ5 if nm == "fittest" else None      # matched azimuth set
...
for fe1 in (False, True):
    a = res[("fittest", fe1)]["coda_db"]
    b = res[("prod5mhz", fe1)]["coda_db"]
    dd = b.mean() - a.mean()
...
np.savez(r"t3_level.npz",
         az5=np.array(AZ5),
         coda2=res[("fittest", False)]["coda_db"],
         coda5=res[("prod5mhz", False)]["coda_db"],
         lin2=res[("fittest", False)]["coda_lin"],
         lin5=res[("prod5mhz", False)]["coda_lin"])
