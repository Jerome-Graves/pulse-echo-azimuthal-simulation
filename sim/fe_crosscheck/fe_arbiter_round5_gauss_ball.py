...
"""Arbiter shot round 5: Gaussian-ball monopole (kills numerical S leak).
...
from fe_arbiter_round2_padded_box import get_mesh, SHELL_FRAC, SHELL_RATE  # noqa: E402
...
def run_p2plus():
    ...
    for tag, ax in (("A", aA), ("B", aB)):
        ...
        out[tag] = (pp.forward(nodes15, tets15, grain, ax13, dt, nt, wav,
                               ...
                               src_type="monopole_ball", rec_type="div",
                               D=D, vol=vol)[:, 0], dt)
...
def main():
    ...
    np.savez("fe_arbiter_round5_gauss_ball_traces.npz", fa=fa, fb=fb, dtf=dtf,
             ea=ea, eb=eb, dte=dte)
    le = analyse(ea, eb, dte, "P2+ (deep src/rec)")
    print(f"\nCROSS-CHECK 5: FDTD {lf:.1f} vs P2+ {le:.1f} dB -> "
