"""Forensics on saved A-B cross-check traces (fe_p2plus_ab*.npz).
...
Usage: python fe_ab_forensics.py [fe_p2plus_ab2_traces.npz]
"""
import sys
...
def main():
    path = sys.argv[1] if len(sys.argv) > 1 else "fe_p2plus_ab2_traces.npz"
    d = np.load(path)
